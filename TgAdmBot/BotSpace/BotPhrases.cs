namespace TgAdmBot.BotSpace
{
    ...
    internal class BotPhrases
    {
        ...
        "Список развлекательных команд\n" +
                            ...
                            "14. /divorce разорвать брак" +
                            "15 /prvt поможет создать секретное сообщение. Синтаксис:" +
                            "\n/prvt 1 @username @seconduser текст который будет скрыт" +
                            "\nЧисло через пробел - это режим. 1 - разрешающий, 2 - запрещающий. " +
                            "Список пользователей, которым будет разрешено (в 1 режиме) и запрещено (во 2 режиме) указывается через пробел после указания режима. " +
                            "Далее следует текст, который будет скрыт.\n" +
                            "\nПри использовании этой команды в лс с ботом необходимо после режима и перед пользователями указать id чата, например:" +
                            "\n/prvt 2 -1901796947119 @username текст\n";
